...
from .models import Company, meta_default_value, get_next_internal_id


class ListUniqueValidator(object):
    """
    This is a custom uniqueness validator for list serializer
    which checks instances against each other before saving them.
    Solution taken from
    https://github.com/encode/django-rest-framework/issues/6395#issuecomment-452412653
    """
...
class CompaniesBulkSerializer(serializers.ListSerializer):
    validators = [ListUniqueValidator(unique_field_names=['object_code'])]
    ...
    def create(self, validated_data):
        ...
        for item in validated_data:
            ...
            item.pop("meta")
            object_data.append(Company(meta=meta_data, **item))

        return Company.objects.bulk_create(object_data)
...
class CompanySerializer(serializers.ModelSerializer):
    object_code = serializers.CharField(max_length=254,
    ...
    def create(self, validated_data):
        ...
        instance = Company.objects.create(meta=meta_data, **validated_data)
    ...
    class Meta:
        model = Company
        fields = ['id', 'object_type', 'name', 'object_code', 'created_date',
                  'modified_date', 'meta', 'data', 'project_id', 'account_id',
                  'user_id', 'object_item', ]
        read_only_fields = ['id', 'created_date', 'modified_date']
        list_serializer_class = CompaniesBulkSerializer
